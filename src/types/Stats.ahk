...
formatStats(statArray) {
    ...
    for statName, statVal in statArray {
        switch (statName) {
            ...
            case "item_tohit_undead_perlevel": statList[statArray["item_tohit_undead_perlevel"].statOrder]:= Format("+{1} to Attack Rating against Undead (Based on Character Level)", Round((statArray["item_tohit_undead_perlevel"].statValue / 8) * playerLevel))
